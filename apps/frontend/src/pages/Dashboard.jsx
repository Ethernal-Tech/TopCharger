...
export default function Dashboard() {
  ...
  const handleMagicLogin = async () => {
    try {
      ...
      // Sign in to NextAuth via Credentials provider "magic"
      const form = new URLSearchParams({ did });
      const res = await fetch(`${BACKEND}/api/auth/callback/magic`, {
        method: "POST",
        headers: { "Content-Type": "application/x-www-form-urlencoded" },
        body: form.toString(),
        credentials: "include",
      });
      if (!res.ok) throw new Error("Magic sign-in failed");
      // Reuse your existing FE flow
      window.location.href = `${FRONTEND}/auth/callback`;
    } catch (e) {
      console.error(e);
      ...
      <div className="relative z-10 flex flex-col items-center justify-start max-w-full sm:max-w-4xl">
        ...
        <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto items-center justify-center">
          ...
          <button
            ...
            className="w-full sm:w-auto px-6 py-3 rounded-lg shadow bg-white/10 border border-white/30 text-white hover:bg-white/15 transition flex items-center justify-center gap-2"
          >
